...
def main():
    ...
    num_workers = 13  # Come da tua richiesta
    ...
    # Vincolo 1: Almeno 2 lavoratori per ogni turno (Requisito Caso A)
    for d in range(num_days):
        for s in range(num_shifts):
            model.add(sum(shifts[(w, d, s)] for w in range(num_workers)) >= 2)
