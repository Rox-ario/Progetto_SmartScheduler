...
- Dizionario dei pesi di soddisfazione (soft): `satisfaction_weights[(w, d, s)]`
  - La matrice viene pre-inizializzata dal sistema con i seguenti pesi di default (Universal Base Hardship):
    ...
    * Turno diurno weekend/fest → -1  (svantaggiato)
    * Turno notturno feriale    → -1  (svantaggiato)
    * Turno notturno weekend    → -2  (molto svantaggiato)
    * Turni in giorni festivi/vacanze generali → -1 (penalizzato di default)
